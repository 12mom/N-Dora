...
    ctx.beginPath();
    const fillWidth = (width * Math.min(progress, 100)) / 100;
    ctx.moveTo(x + radius, y);
    ctx.lineTo(x + fillWidth - radius, y);
    ctx.quadraticCurveTo(x + fillWidth, y, x + fillWidth, y + radius);
    ctx.lineTo(x + fillWidth, y + radius);
    ctx.quadraticCurveTo(x + fillWidth, y + radius * 2, x + fillWidth - radius, y + radius * 2);
    ctx.lineTo(x + radius, y + radius * 2);
    ctx.quadraticCurveTo(x, y + radius * 2, x, y + radius);
    ctx.lineTo(x, y + radius);
    ctx.quadraticCurveTo(x, y, x + radius, y);
...
        // ضبط حجم الخط تلقائيًا حسب طول الاسم
        let fontSize = 20;
        ctx.font = `bold ${fontSize}px sans-serif`;
        while (ctx.measureText(name).width > 200 && fontSize > 10) {
            fontSize -= 1;
            ctx.font = `bold ${fontSize}px sans-serif`;
        }

        // اختيار لون الاسم حسب الرتبة
        let nameColor = '#ffffff';
        if (rank === 1) nameColor = '#FFD700'; // ذهبي
        else if (rank === 2) nameColor = '#C0C0C0'; // فضي
        else if (rank === 3) nameColor = '#CD7F32'; // برونزي
...
        ctx.fillStyle = '#ffffff';
        let rankSymbol = rank === 1 ? '🥇' : rank === 2 ? '🥈' : rank === 3 ? '🥉' : '';
        ctx.fillText(`Rank ${rank} ${rankSymbol}`, 136, 66);
...
        percent = Math.max(0, Math.min(percent, 100)); // ضبط النسبة بين 0 و 100
...
                senderExp,
                senderRank,
                allData: (_listOf == 'global' ? allData_withName.slice(0, 20) : allData_withName)
                    .map((e, i) => `${i + 1}. ${e.name} (${e.userID}) - ${e.exp} exp`).join('\n')
            }))
        }
...
    try { if (savePath && global.isExists(savePath)) global.deleteFile(savePath); } catch (e) { console.error(e); }
    try { if (avatarPath && global.isExists(avatarPath)) global.deleteFile(avatarPath); } catch (e) { console.error(e); }
}
